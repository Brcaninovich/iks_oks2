...
public class BotActivity extends AppCompatActivity {
    ...
    public void set_value(View view) {
        ...
        if(!zavrsenGejm){
            ...
            String test = gameplay.pobjeda_bot(binding);
            if(test == null){
                if(brojac >= 9){
                    binding.winnerTV.setText("DRAW");
                    gameplay.winner_bot(binding, 2);
                    zavrsenGejm = true;
                }
            }else{
                binding.winnerTV.setText(test);
                gameplay.winner_bot(binding, 1);
                zavrsenGejm = true;
    ...
    public void restart_game(View view) {
        navigateUpTo(new Intent(BotActivity.this, BotActivity.class));
